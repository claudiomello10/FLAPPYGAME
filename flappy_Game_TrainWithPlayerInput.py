...
def run():
    ...
    while game_active == True:
        ...
        full_x_data.append(pipe_parameters)
        full_y_data.append(1 if jumped else 0)

        Clock.tick(60)
        pygame.display.update()
    full_x_data = np.array(full_x_data)
    full_y_data = np.array(full_y_data)
    return (full_x_data[:-100], full_y_data[:-100])
...
x_result, y_result = run()
print(x_result.shape, y_result.shape)
np.save("NN_playerTrain_x_data.npy", x_result)
np.save("NN_playerTrain_y_data.npy", y_result)
